// Source: https://github.com/tickbh/rbtree-rs
...
impl<K: Ord, V> NodePtr<K, V> {
    ...
    fn seek_old(self, cmp_fn: impl Fn(&K) -> bool) -> NodePtr<K, V> {
        ...
        // Step 1: Walk up the tree to find a candidate node not satisfying cmp_fn
        let mut current = self;
        let mut candidate = self.parent();
        while !candidate.is_null() && cmp_fn(unsafe { &(*candidate.0).key }) {
            current = candidate;
            candidate = candidate.parent();
        }

        // We have that `cmp_fn(current)` is true, and `cmp_fn(candidate)` is false or it's null.
        // Step 2: Perform a binary search from `current` for the least key satisfying cmp_fn.
    ...
    fn seek(self, cmp_fn: impl Fn(&K) -> bool) -> NodePtr<K, V> {
        ...
        // Let `ancestor` be the least ancestor of `candidate` greater than it.
        // We maintain the following loop invariants:
        // 1. If target < ancestor, either target = candidate or target is a descendant of current
        // 2. current < ancestor
        ...
        // However, for efficiency, we instead write this as a loop with explicit breaks when the condition is false
...
impl<'a, K: Ord + Copy, V> IntoStreamIterator for &'a RBTree<K, V> {
    ...
    fn into_stream_iterator(self) -> IndexedStreamIter<'a, K, V> {
        self.stream_iter()
    }
}
